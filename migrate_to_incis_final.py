...
if PLACEHOLDER:
    abstract_text = (
        'Decentralized edge swarms operating in Denied, Disrupted, Intermittent, and Low-Bandwidth (DDIL) environments face severe '
    ...
    abstract_text = (
        f'Decentralized edge swarms operating in Denied, Disrupted, Intermittent, and Low-Bandwidth (DDIL) environments face severe '
    ...
    f' The complete artifact (source code, per-run CSVs, seeds, and reproduction scripts) is anonymously available at https://anonymous.4open.science/r/Swarm-coord-in-ddil.'
    )
...
add_heading1('VI. Open Science')
add_body(
    'In the spirit of open science and to ensure full reproducibility of the reported results, the authors commit to making '
    'all artifacts from this work publicly available upon publication. The complete source code for the simulation framework, '
    'the per-run canonical CSVs (benchmark, ablation, sensitivity, and robustness suites), the deterministic reproduction '
    'scripts, the figure generation code, and the paper manuscript source are available in the anonymized repository at '
    'https://anonymous.4open.science/r/Swarm-coord-in-ddil. The authors are committed to maintaining this archive and '
    'ensuring that all numerical claims in the paper can be independently verified by any researcher without restriction.'
)

add_heading1('VII. Threats to Validity and Limitations')
add_bullet(
...
add_heading1('VIII. Conclusion and Future Work')
add_body(
